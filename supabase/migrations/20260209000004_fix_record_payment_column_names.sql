...
-- =============================================================================
-- Fix record_payment RPC: Ensure correct column names match payments schema
-- =============================================================================
-- This migration ensures the record_payment function explicitly uses:
-- - payment_method (not "method")
-- - All other columns match the payments table schema exactly
-- - received_by is explicitly set (in addition to trigger)
-- =============================================================================
...
BEGIN
  ...
  INSERT INTO public.payments (
    job_id,
    amount,
    payment_method,  -- Correct column name (not "method")
    paid,
    date_paid,
    notes,
    company_id,
    ...
    status,
    created_by,
    received_by,      -- Explicitly set (trigger also handles this)
    external_ref,
    invoice_id
  )
  VALUES (
    p_job_id,
    p_amount,
    p_method,         -- Maps to payment_method column
    true,
    CURRENT_DATE,
    p_notes,
    v_company_id,
    ...
    'posted',
    auth.uid(),
    auth.uid(),       -- Explicitly set received_by to current user
    p_external_ref,
    p_invoice_id
  )
  RETURNING
