...
-- =============================================================================
-- Add invoice_id to payments table and update record_payment RPC
-- Supports true invoice lifecycle while maintaining backward compatibility
-- =============================================================================

-- 1) Add invoice_id column to payments (nullable for backward compatibility)
ALTER TABLE public.payments
  ADD COLUMN IF NOT EXISTS invoice_id uuid;

-- 2) Add foreign key constraint (ON DELETE SET NULL to preserve payment history)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
-- 3) Create helpful indexes for invoice-based queries
CREATE INDEX IF NOT EXISTS idx_payments_company_invoice_id
...
-- 4) Update record_payment RPC to accept optional invoice_id
-- Preserves all existing logic (team-based assignment, audit logging, etc.)
...
BEGIN
  ...
  -- If invoice_id is provided, validate it exists and matches company + job
...
-- 5) Grant execute to authenticated (idempotent)
