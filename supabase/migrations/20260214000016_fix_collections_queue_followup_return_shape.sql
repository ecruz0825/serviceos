-- =============================================================================
-- Fix Collections Queue Follow-up Return Shape (Phase C5 Fix)
-- =============================================================================
-- Ensures get_collections_queue_for_company RETURNS TABLE and SELECT both
-- include next_followup_at and followup_due fields appended at the end.
-- =============================================================================

BEGIN;

-- Drop function with exact signature
DROP FUNCTION IF EXISTS public.get_collections_queue_for_company(integer, timestamptz);

-- Recreate function with follow-up fields explicitly in signature and SELECT
...
BEGIN
  ...
  SELECT
    ...
    -- Follow-up fields (appended at end - MUST match RETURNS TABLE order)
