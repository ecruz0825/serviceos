-- =============================================================================
-- Crew-Specific Route Optimization
-- =============================================================================
-- Adds optional crew filtering to route optimization RPCs
-- When p_assigned_team_id is provided, optimize only jobs for that crew
-- When p_assigned_team_id is NULL, preserve existing behavior (all jobs)
-- =============================================================================

BEGIN;

-- =============================================================================
-- Part 1: Update get_optimized_route_for_day to support optional crew filter
-- =============================================================================
...
BEGIN
  ...
  -- Filter by crew if p_assigned_team_id is provided
  ...
  WHERE j.company_id = v_company_id
    ...
    AND (
      p_assigned_team_id IS NULL 
      OR j.assigned_team_id = p_assigned_team_id
    );

  -- No routeable jobs for this day/crew
...
-- =============================================================================
-- Part 2: Update apply_optimized_route_for_day to support optional crew filter
-- =============================================================================
...
BEGIN
  ...
  -- 5) Apply optimized route order for caller company + selected date + optional crew
  WITH optimized AS (
    SELECT *
    FROM public.get_optimized_route_for_day(p_service_date, p_assigned_team_id)
  ),
  updated AS (
...
-- Grant permissions remain the same (functions are replaced, not recreated)
-- GRANT EXECUTE ON FUNCTION public.get_optimized_route_for_day(date, uuid) TO authenticated;
-- GRANT EXECUTE ON FUNCTION public.apply_optimized_route_for_day(date, uuid) TO authenticated;
