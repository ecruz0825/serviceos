...
-- - Deterministic role assignment (never falls back to invalid role='user')
-- - Safe metadata parsing with validation
-- - Crew member auto-linking by email
-- - Company ID validation before insert
-- =============================================================================
...
BEGIN
  ...
  IF v_role IN ('admin', 'crew', 'customer') THEN
  ...
  -- 8) Insert profile with validated data
