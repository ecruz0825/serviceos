-- =============================================================================
-- HOTFIX: Fix handle_new_user() unsafe record variable dereferencing
-- =============================================================================
-- Problem: v_crew_member record was being set to NULL and then dereferenced,
-- which is unsafe in PL/pgSQL and can cause auth user creation failures.
--
-- Solution: Replace `v_crew_member record` with explicit scalar variables:
--   - v_crew_member_id uuid
--   - v_crew_member_company_id uuid
--   - v_crew_member_full_name text
--   - v_crew_member_role text
--
-- This eliminates all record field dereferencing on potentially-null records.
-- =============================================================================
...
DECLARE
  ...
  v_role text;
  v_full_name text;
  -- Explicit scalar variables instead of record (safer - no null record dereferencing)
  v_crew_member_id uuid;
  v_crew_member_company_id uuid;
  v_crew_member_full_name text;
  v_crew_member_role text;
...
BEGIN
  ...
  -- Initialize scalar variables to NULL (safe - no record dereferencing issues)
  v_crew_member_id := NULL;
  v_crew_member_company_id := NULL;
  v_crew_member_full_name := NULL;
  v_crew_member_role := NULL;
  ...
  IF v_metadata_crew_member_id IS NOT NULL THEN
    BEGIN
      ...
      INTO v_crew_member_id, v_crew_member_company_id, v_crew_member_full_name, v_crew_member_role
      ...
      IF FOUND AND v_company_id IS NOT NULL AND v_crew_member_company_id <> v_company_id THEN
        RAISE WARNING 'Crew member % does not belong to company % (belongs to %)', 
          v_crew_member_id, v_company_id, v_crew_member_company_id;
        -- Clear crew member data on mismatch
        v_crew_member_id := NULL;
        v_crew_member_company_id := NULL;
        v_crew_member_full_name := NULL;
        v_crew_member_role := NULL;
  ...
  IF v_crew_member_id IS NULL AND (v_metadata_role IS NULL OR v_company_id IS NULL) AND NEW.email IS NOT NULL THEN
    SELECT cm.id, cm.company_id, cm.full_name, cm.role
    INTO v_crew_member_id, v_crew_member_company_id, v_crew_member_full_name, v_crew_member_role
    FROM public.crew_members cm
    ...
    IF FOUND THEN
      IF v_company_id IS NULL THEN
        v_company_id := v_crew_member_company_id;
      END IF;
      IF v_metadata_role IS NULL THEN
        v_role := COALESCE(v_crew_member_role, 'crew');
      ELSE
        v_role := v_metadata_role;
      END IF;
      IF v_metadata_full_name IS NULL THEN
        v_full_name := v_crew_member_full_name;
      ELSE
  ...
  ELSE
    ...
    IF v_crew_member_id IS NOT NULL THEN
      IF v_company_id IS NULL THEN
        v_company_id := v_crew_member_company_id;
      END IF;
      IF v_metadata_full_name IS NULL THEN
  ...
  IF v_role IN ('admin', 'crew', 'customer', 'manager', 'dispatcher', 'platform_admin') THEN
    NULL; -- v_role already set to valid value
  ELSIF v_crew_member_id IS NOT NULL THEN
    v_role := COALESCE(v_crew_member_role, 'crew');
  ELSE
    v_role := 'customer';
  END IF;
  ...
  IF v_crew_member_id IS NOT NULL THEN
    BEGIN
      ...
      WHERE id = v_crew_member_id
        ...
        AND company_id = COALESCE(v_company_id, v_crew_member_company_id);
    EXCEPTION WHEN OTHERS THEN
      RAISE WARNING 'Failed to link crew_member % to user %: %', v_crew_member_id, NEW.id, SQLERRM;
    END;
  END IF;
