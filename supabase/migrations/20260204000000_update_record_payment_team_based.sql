...
-- Update record_payment() RPC to use team-based assignment instead of assigned_to
...
BEGIN
  ...
  IF v_role = 'crew' THEN
    ...
    -- Check if job has assigned_team_id
    ...
    ) INTO v_user_is_on_team;

    IF NOT v_user_is_on_team THEN
      RAISE EXCEPTION 'forbidden_job_not_assigned_to_team';
    END IF;
  END IF;
